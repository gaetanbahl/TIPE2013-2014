...
\section{Transformation en ondelettes discrètes : application à la compression des données}
...
Le code complet de nos algorithmes est disponible en annexe à la fin de ce dossier, mais aussi sur le Github de notre projet (cf. section Liens). Les langages utilisés sont Python (dans un premier temps) et OpenCL (dans la dernière partie).
...
Nous avons mis au point un autre algorithme de compression n'utilisant pas de matrices. Celui-ci est plus rapide et moins gourmand en mémoire que @haar@, mais l'inconvénient est qu'il n'est plus possible d'appliquer l'algorithme récursivement. Il suffit cependant à compresser rapidement des images.


\subsubsection{Fonctionnement}

L'algorithme traite l'image par carrés de 4 pixels, qu'il va traiter itérativement, colonne par colonne. Comme montré sur la figure \ref{algo}. C'est en ce sens que l'algorithme prend moins de place en mémoire. Il n'utilise qu'un tableau contenant 4 pixels au lieu de stocker toute l'image dans une matrice. Il accède au fichier de l'image à l'aide de la librairie PIL (Python Imaging Library), qui permet d'accéder directement aux données RGB des pixels.
...
La figure \ref{after} montre les pixels avant et après traitement. Les pixels peuvent sembler réellement différents après traitement, mais la valeur moyenne des couleurs est conservée, ce qui, au final, donnera le même rendu quand les pixels seront à leur taille normale.
